    ...
    return (
        <>
            <motion.div
                ...
                className="fixed inset-0 w-full min-h-screen sm:h-[90vh] md:h-[600px] backdrop-blur-lg rounded-none sm:rounded-lg md:rounded-xl overflow-hidden z-50"
            >
                    ...
                    <div className="flex-1 p-2 sm:p-3 md:p-4 flex items-center justify-center bg-black/80">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 md:p-4 bg-gradient-to-t from-black/80 to-transparent text-left">
                ...
                <motion.button
                    className="absolute top-4 right-4 p-2 rounded-full bg-white/20 text-white hover:bg-white/40 text-xs sm:text-sm backdrop-blur-md"
                    onClick={onClose}
                    ...
                    <div className="flex items-center -space-x-2 px-3 py-2">
                        {mediaItems.map((item, index) => (
                            <motion.div
                                ...
                                className={`relative group w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex-shrink-0 rounded-lg overflow-hidden cursor-pointer hover:z-20 ${selectedItem.id === item.id ? 'ring-2 ring-amber-500 shadow-lg' : 'hover:ring-2 hover:ring-white/30'}`}
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-4xl text-left">
            <div className="mb-8 text-center">
                <motion.h3
                    className="text-xl sm:text-2xl md:text-3xl font-bold text-emerald-900"
                    ...
                    >
                        {items.map((item, index) => (
                            <motion.div
                                ...
                                className={`relative overflow-hidden rounded-xl cursor-zoom-in border border-stone-200 shadow-xs bg-stone-100 ${item.span}`}
                            ...
                            >
                                <MediaItem
                                    ...
                                    onClick={() => !isDragging && setSelectedItem(item)}
                                />
                                <div className="absolute inset-0 flex flex-col justify-end p-3 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-none">
                                    <h4 className="text-white text-xs font-bold line-clamp-1">{item.title}</h4>
                                    <p className="text-white/70 text-[10px] line-clamp-1 mt-0.5">{item.desc}</p>
                                </div>
                            </motion.div>
                        ))}
                    </motion.div>
